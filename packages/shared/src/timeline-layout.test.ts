...
import { axisTicks, ribbonRows, spansBounds } from './timeline-layout';
...
describe('ribbonRows', () => {
	...
	it('positions a span as a left offset and width in percent of the axis', () => {
		const [row] = ribbonRows([span('React', 2012, 2017)], bounds);
	...
	it('resolves a present end to the axis max', () => {
		const [row] = ribbonRows([span('React', 2015, 'present')], bounds);
		const [segment] = row.segments;
	...
	it('floors a non-brief bar to a minimum readable width', () => {
		const [row] = ribbonRows([span('React', 2015, 2015)], bounds); // zero-length span
		const [segment] = row.segments;
		expect(segment.width).toBe(2);
	});

	it('pulls a floored bar back so it never overflows the right edge', () => {
		const [row] = ribbonRows([span('React', 2020, 2020)], bounds); // at the very edge
		const [segment] = row.segments;
	...
	it('does not floor or clamp a brief segment (rendered as a fixed-size dot)', () => {
		const [row] = ribbonRows([span('React', 2020, 2020, 'brief')], bounds);
		const [segment] = row.segments;
	...
	it('avoids a divide-by-zero when min equals max', () => {
		const [row] = ribbonRows([span('React', 2010, 2010)], { min: 2010, max: 2010 });
		const [segment] = row.segments;
...
describe('axisTicks', () => {
	it('emits evenly spaced markers aligned to the step across the bounds', () => {
		const ticks = axisTicks({ min: 2010, max: 2020 }, 5);
